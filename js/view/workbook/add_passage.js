...
function pageinit(isHelloBook, memberId, isSsam) {
	...
	const masonryOptsForPassages = {
		itemSelector: '.passage', columnWidth: '.passage',
	...
	function _verifyUsageLimit() {
		if (!isSsam && memberId != 15000550 && memberId != 15000590 && memberId != 15000998 && memberId != 15001122 && memberId != 15001021 && memberId != 15001905 && memberId != 15001515 && memberId!= 15002176 && memberId != 15002181 && memberdId != 15002185 && memberId != 15002196 && myFicoUsages.length >= MAX_SENTENCE_LENGTH_PER_DAY) {
	...
	function replaceAndHighlights() {
		...
		$('#newPassageText').highlightWithinTextarea({
			highlight: [
...
/*				if (searchingSentenceDone) {
	...
	$('#addBtn').on('click', async function() {
		...
		if (isHelloBook) { // HelloBook 모드인 경우
			...
			if (sentences.reduce((acc, curr) => acc + curr.length, 0) > maxChars) { // 입력한 문장이 최대 글자수를 초과하는 경우
				alertModal(`${maxChars}자가 넘는 문장은\n워크북에서 등록하실 수 있습니다.`);
				return;
			}
			createHidden($form, 'text', sentences.join('\n'));
		} else {
			$form[0].action = '/workbook/passage/add'; // 기본 액션 설정
			if ($('#title').val().length == 0) { // 타이틀이 비어있는 경우
		...
		myFicoUsages.length += sentencesLength; // 사용된 문장의 길이를 기록
		localStorage.setItem(MY_FICO_USAGES_KEY, btoa(JSON.stringify(myFicoUsages)));
