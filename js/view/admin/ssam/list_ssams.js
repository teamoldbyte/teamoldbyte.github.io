/** /admin/ssam/list_ssams.html
@author LGM
 */
async function pageinit() {
	const ssamPaginationContainer = document.querySelector('#ssamPagination');
	const ssamListContainer = document.querySelector('#ssamListDiv tbody');
	...
	*/
	...
	$(document).on('click','.thlink[data-value]', function() {
		...
		const searchFormId = ssamListContainer.dataset.searchForm;
	...
	$(document).on('click','.page-link', function() {
		const searchFormId = ssamPaginationContainer.dataset.searchForm;
	...
	.on('shown.bs.collapse', '#ssamDetailSection', function() {
		window.scrollTo(0,document.getElementById('ssamDetailSection').offsetTop);
	})
