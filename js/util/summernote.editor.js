...
(function($, bootstrap, window, document) {
	...
	async function openSummernote($input) {
		if (typeof $.summernote == 'undefined') {
			...
			customstyle.innerHTML = '.note-editor.note-frame {-webkit-user-select: initial;user-select: initial;}.note-toolbar .dropdown-toggle::after{display:none;} .note-editable{font-family: "HCRDotum";white-space: break-spaces;tab-size: 4}@media(min-width: 576px){.note-editable{background:repeating-linear-gradient(transparent,transparent 10px, #fff 10px, #fff 20px), linear-gradient(to right, transparent calc(320px - 1px), #eee 320px, transparent calc(320px + 1px));}.note-editable::before{content: "모바일 경계선";position: absolute;color: #bbb;font-size: 5px;left: 320px;top: 0;transform: translateX(-50%);}}.note-editable::after{position:absolute;right: 20px;top: 0;content:attr(data-char-count);font-size: 5px}.note-editable.note-overlimit::after{color: #f00}';
